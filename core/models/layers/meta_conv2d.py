...
class _MetaConv2dMultiPath(Meta_Layer_BASE):
    def __init__(
        ...
        lambda_ = 1550,
        # LUT_path: str = "../../LUT.npy",
        # approximated_function: str = "../../functions.npy",
        ...
        # self.approximated_function=approximated_function
        # self.reinforce_training = reinforce_training
        # self.parallel_path = parallel_path
        self.sigma_trainable = sigma_trainable
        self.path_multiplier = path_multiplier
        self.transfer_matrix_initialize()
        # self.lut = np.load(self.LUT_path)
        # self.functions = np.load(self.approximated_function)
    ...
    def build_parameters(self, bias: bool) -> None:
        ...
        self.alm_multiplier = nn.Parameter(
            torch.empty(self.path_depth, 2, self.path_multiplier, device=self.device), requires_grad=False
        )
    ...
    def build_path_weight(self):
        ...
        path_weight = path_weight / path_weight.data.sum(dim=1, keepdim=True)  # Sum over rows
        # print(path_weight.shape)
        path_weight = path_weight / path_weight.data.sum(dim=2, keepdim=True)  # Sum over columns

        with torch.no_grad():
            perm_loss = (
                path_weight.data.norm(p=1, dim=1).sub(path_weight.data.norm(p=2, dim=1)).mean(dim=-1) +
                (1 - path_weight.data.norm(p=2, dim=2)).mean(dim=(-1))
            )
    ...
    def reset_parameters(self):
        ...
        nn.init.constant_(self.alpha.data, 0.02)
    ...
    def get_alm_perm_loss(self, rho: float = 0.1):
        ...
        for i in range(path_weight.shape[0]):
            loss_r = self.alm_multiplier[i, 0].dot(d_path_weight_r[i] + rho / 2 * d_path_weight_r_square[i])
            loss_c = self.alm_multiplier[i, 1].dot(d_path_weight_c[i] + rho / 2 * d_path_weight_c_square[i])
    ...
    def update_alm_multiplier(self, rho: float = 0.1, max_lambda: Optional[float] = None):
        with torch.no_grad():
            ...
            d_path_weight_r = path_weight.norm(p=1, dim=1).sub(path_weight.norm(p=2, dim=1))
            d_path_weight_c = path_weight.norm(p=1, dim=2).sub(path_weight.norm(p=2, dim=2))
            d_path_weight_r_square = d_path_weight_r.square()
            d_path_weight_c_square = d_path_weight_c.square()
            for i in range(path_weight.shape[0]):
                self.alm_multiplier[i, 0].add_(d_path_weight_r[i] + rho / 2 * d_path_weight_r_square[i])
                self.alm_multiplier[i, 1].add_(d_path_weight_c[i] + rho / 2 * d_path_weight_c_square[i])
    ...
    def path_generation(self, path_weight):
        ...
        full_repetitions, remainder = divmod(self.in_channels, path_after_transpose.size(0))
        repeated_a = path_after_transpose.repeat(full_repetitions, 1)
        if remainder:
            repeated_a = torch.cat((repeated_a, path_after_transpose[:remainder]), dim=0)
    ...
    def construct_positions(self, k):
        ...
        positions = torch.stack([grid_x.flatten(), grid_y.flatten()],
                                dim=1)  # Convert to float for distance calculations

        return positions

    def transfer_matrix_initialize(self):
        k = self.kernel_size[0]
        comj = torch.tensor(1j)
        # Coordinates for the first matrix

        coords1 = self.construct_positions(k)
        coords2 = self.construct_positions(k)
        ...
        coords_differences = coords1_expanded - coords2_expanded  # Resulting shape will be [k, k, 2]
        ...
        squared_distances = torch.sum(coords_differences ** 2, dim=2) + 1

        # Take the square root to get the Euclidean distances
        distances_efficient = torch.sqrt(squared_distances)

        self.transfer_matrix = (1 / (2 * np.pi)) * (1 / distances_efficient) * (1/distances_efficient - self.wv * comj) \
                               * (torch.exp(comj * self.wv * distances_efficient) / distances_efficient)

        self.transfer_matrix = self.transfer_matrix.to(self.device)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # x = torch.unsqueeze(2)
        # print(x.shape)
        # x_extend = x_extend.repeat(1, 1, 2, 1, 1)
        # x = x_extend
        print(f"Before the loop, Norm -:  {torch.norm(x.detach()).item()}")
        ...
        # weight[:, :, 0, :, :] = weight[:, :, 0, :, :] * beta
        # weight[:, :, 1, :, :] = weight[:, :, 1, :, :] * (1 - beta)
        ...
        x = torch.nn.functional.interpolate(x, size=self.kernel_size, mode="nearest")
        ...
        print(f"Before the loop, Norm 0:  {torch.norm(x.detach()).item()}")

        print(x.device)  # Prints the device of tensor x
        print(self.transfer_matrix.device)
        for i in range(self.path_depth):
            ...
            print(f"Loop 1, Norm {i}:  {torch.norm(x.detach()).item()}")
            bs_s, outc_s, inc_s, xy_s, ker_s, _ = x.shape
            x = x.view(-1, ker_s**2)  # [bs, outc, inc, 2, s^2]
            x = torch.matmul(x, self.transfer_matrix)
            x = x.view(bs_s, outc_s, inc_s, xy_s, ker_s, ker_s) #[bs, outc, inc, 2, s, s]
        ...
        # print(x_wise_real.shape, y_wise_imag.shape)
        ...
        #     x.imag, size=input_size, mode="nearest"
        # )  # [bs*outc, inc, 2, h, w]real
        # # photodetection
        # print(beta.shape)
        # print(alpha)
        x_wise = (x_wise_real.square() + x_wise_imag.square()) * beta # x_wise detection
        y_wise = (y_wise_real.square() + y_wise_imag.square()) * (1 - beta) # y_wise detection

        # print(x_wise.shape, y_wise.shape)
        ...
        print(f"Final, Norm:  {torch.norm(x.detach()).item()}")
