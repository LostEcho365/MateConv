...
# def update_batchnorm(bn_layer, new_num_channels):
#     bn_layer.running_mean = bn_layer.running_mean[:new_num_channels]
#     bn_layer.running_var = bn_layer.running_var[:new_num_channels]
#     bn_layer.weight = nn.Parameter(bn_layer.weight[:new_num_channels])
#     bn_layer.bias = nn.Parameter(bn_layer.bias[:new_num_channels])
#     bn_layer.num_features = new_num_channels
#

def resnet_unstructural_prune(model, epoch, total_epochs, initial_pruning_rate=0.1, final_pruning_rate=0.2):
    # Linearly increase the pruning rate over epochs
    pruning_rate = initial_pruning_rate + (final_pruning_rate - initial_pruning_rate) * epoch / total_epochs

    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            # Calculate the pruning mask for each filter
            norms = torch.norm(module.weight, p=1, dim=[1, 2, 3])  # L1 norm across each filter
            threshold = torch.quantile(norms, pruning_rate)
            mask = norms > threshold

            # Expand the mask for element-wise multiplication
            expanded_mask = mask[:, None, None, None].expand_as(module.weight)

            # Apply pruning mask to Conv2d layer by zeroing out the weights
            module.weight.data *= expanded_mask

            # Optionally, also prune the biases associated with the pruned filters
            if module.bias is not None:
                bias_pruning_mask = mask
                expanded_bias_mask = bias_pruning_mask.expand_as(module.bias)
                module.bias.data *= expanded_bias_mask

def resnet_structural_prune(model, epoch, total_epochs, initial_pruning_rate=0.1, final_pruning_rate=0.2):
    # Linearly increase the pruning rate over epochs
    pruning_rate = initial_pruning_rate + (final_pruning_rate - initial_pruning_rate) * epoch / total_epochs
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and 'shortcut' not in name:
            ...
            norms = torch.norm(module.weight, p=1, dim=[1, 2, 3])
            threshold = torch.quantile(norms, pruning_rate)
            mask = norms > threshold

            # Apply pruning mask to Conv2d layer
            module.weight.data = module.weight.data[mask, :, :, :]
            module.weight.requires_grad = True
            if module.bias is not None:
                module.bias.data = module.bias.data[mask]
                module.bias.requires_grad = True
            # module.out_channels = module.weight.data.size(0)
            # Update the corresponding BatchNorm layer
            # bn_name = name.replace('conv', 'bn')
            # if bn_name in module_names:
            #     bn_module = dict(model.named_modules())[bn_name]
            #     bn_module.weight.data = bn_module.weight.data[mask]
            #     bn_module.bias.data = bn_module.bias.data[mask]
            #     bn_module.running_mean = bn_module.running_mean[mask]
            #     bn_module.running_var = bn_module.running_var[mask]
            #     bn_module.num_features = sum(mask)
            #
            #
            # if previous_output_channel:
            #     module.in_channels = previous_output_channel
            #
            # previous_output_channel = sum(mask)
            #
            # if 'conv1' in name:
            #     input_channel_blockwise = module.in_channels
            #
            # if 'conv2' in name:
            #     output_channel_blockwise = sum(mask)
        #
        # elif isinstance(module, nn.Conv2d):
        #     module.in_channels = input_channel_blockwise
        #     module.out_channels = output_channel_blockwise
        #     new_conv = nn.Conv2d(input_channel_blockwise, output_channel_blockwise, kernel_size=module.kernel_size, stride=module.stride,
        #                          padding=module.padding, bias=(module.bias is not None))
        #     nn.init.constant_(new_conv.bias, 0)
        #     bn_name = name.replace('conv', 'bn')
        #     if bn_name in module_names:
        #         new_bn = nn.BatchNorm2d(output_channel_blockwise)
        #         nn.init.constant_(new_bn.weight, 1)
        #         nn.init.constant_(new_bn.bias, 0)
        #         model._modules[name] = new_bn
        #
        #
        # elif isinstance(module, nn.Linear):
        #     module.in_features = previous_output_channel
...
def train(
    ...
    if epoch > 25 and epoch % 10 == 0:
        resnet_unstructural_prune(model, epoch, 100)
...
def main() -> None:
    ...
    epoch = 0
    try:
        get_hostname = lambda: os.environ['COMPUTERNAME'] if platform.system() == "Windows" else os.uname()[1]
        lg.info(
            f"Experiment {configs.run.experiment} ({experiment.experiment_id}) starts. Run ID: ({mlflow.active_run().info.run_id}). PID: ({os.getpid()}). PPID: ({os.getppid()}). Host: ({get_hostname})"
        )
        lg.info(configs)
